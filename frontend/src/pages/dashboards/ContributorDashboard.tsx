...
// Pastikan Anda sudah membuat file service ini sesuai instruksi sebelumnya
...
const ContributorDashboard: React.FC = () => {
    ...
    const [students, setStudents] = useState<any[]>([]); // List Siswa
    const [classes, setClasses] = useState<string[]>([]); // List Kelas
    const [historyData, setHistoryData] = useState<any[]>([]); // Riwayat Aktivitas
    ...
    const [misiForm, setMisiForm] = useState({
        targetType: 'siswa', // 'siswa' or 'kelas'
        targetId: '', // ID siswa atau Nama Kelas
        title: '',
        dueDate: ''
    });
    ...
    const loadInitialData = async () => {
        try {
            ...
            await loadHistory(); // Load history sekalian
        } catch (error) {
            // Error handling silent agar tidak mengganggu UX
    ...
    // [PERBAIKAN] Fungsi Logout yang Mulus (Tanpa Glitch)
    const handleLogout = () => {
    ...
    const handleJadwalkanMisi = async () => {
        ...
        if (misiForm.targetType === 'siswa') {
            ...
            payload.targetClass = misiForm.targetId;
        }
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <main className="flex-1 overflow-auto p-4 md:p-8">
                    <div className="max-w-5xl mx-auto pb-20">
                            ...
                            <div className="space-y-8 animate-fade-in">
                                <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
                                    ...
                                    <p className="text-emerald-100 max-w-2xl text-lg font-medium relative z-10">
                                        Sebagai kontributor, Anda berperan penting dalam memberikan penilaian sikap dan tantangan (misi) untuk membentuk karakter siswa.
                                    </p>
                                </div>
                                    ...
                                    {habits.map((habit, idx) => (
                                        <div key={idx} className={`p-6 rounded-2xl border bg-white shadow-sm hover:shadow-md transition-all`}>
                                            ...
                                            <div className={`p-3 rounded-xl ${habit.color} bg-opacity-20 border border-opacity-20`}>
                                                <p className="text-[10px] font-bold uppercase mb-1 opacity-80 flex items-center gap-1"><Target size={12}/> Indikator</p>
                        ...
                        {/* TAB 2: MISI & PENILAIAN */}
                        {activeTab === 'misi' && (
                            <div className="space-y-6 animate-fade-in">
                                {/* Sub-Tab Nav */}
                                <div className="flex bg-white p-1 rounded-xl shadow-sm border border-gray-200 w-fit mb-6">
                                    <button 
                                        onClick={() => setActiveMissionTab('sikap')}
                                        className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${activeMissionTab === 'sikap' ? 'bg-emerald-100 text-emerald-700 shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}
                                    >
                                        <div className="flex items-center gap-2"><Award size={16}/> Penilaian Sikap</div>
                                    </button>
                                    <button 
                                        onClick={() => setActiveMissionTab('jadwal')}
                                        className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${activeMissionTab === 'jadwal' ? 'bg-emerald-100 text-emerald-700 shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}
                                    >
                                    ...
                                    <div className="bg-white p-6 md:p-8 rounded-3xl shadow-lg border border-gray-100 max-w-2xl">
                                        ...
                                        <div className="space-y-5">
                                            <div>
                                                ...
                                                <select 
                                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl font-bold text-gray-700 outline-none focus:border-emerald-500"
                                                    value={sikapForm.role}
                                                    onChange={(e) => setSikapForm({...sikapForm, role: e.target.value})}
                                                >
                                            ...
                                            <div>
                                                ...
                                                <select 
                                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl font-bold text-gray-700 outline-none focus:border-emerald-500"
                                                    value={sikapForm.studentId}
                                                    onChange={(e) => setSikapForm({...sikapForm, studentId: e.target.value})}
                                                >
                                            ...
                                            <div>
                                                ...
                                                <select 
                                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl font-bold text-gray-700 outline-none focus:border-emerald-500"
                                                    value={sikapForm.category}
                                                    onChange={(e) => setSikapForm({...sikapForm, category: e.target.value})}
                                                >
                                            ...
                                            <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex gap-3 items-start">
                                                ...
                                                <div>
                                                    <p className="text-sm font-bold text-blue-800">TANGGAL PENILAIAN</p>
                                                    <input 
                                                        type="date" 
                                                        className="bg-transparent border-b border-blue-300 text-blue-900 font-bold text-sm outline-none mt-1"
                                                        value={sikapForm.date}
                                                        onChange={(e) => setSikapForm({...sikapForm, date: e.target.value})}
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <input 
                                                    ...
                                                    value={sikapForm.score}
                                                    onChange={(e) => setSikapForm({...sikapForm, score: parseInt(e.target.value)})}
                                                />
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <textarea 
                                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl font-medium text-sm text-gray-700 outline-none focus:border-emerald-500"
                                                    rows={2}
                                                    placeholder="Contoh: Sangat sopan saat bertanya..."
                                                    value={sikapForm.notes}
                                                    onChange={(e) => setSikapForm({...sikapForm, notes: e.target.value})}
                                                />
                                            </div>

                                            <button 
                                                onClick={handleKirimSikap}
                                                className="w-full py-4 bg-emerald-600 text-white rounded-xl font-bold text-lg hover:bg-emerald-700 transition-all shadow-lg hover:shadow-emerald-200 flex justify-center items-center gap-2"
                                            >
                                ...
                                {activeMissionTab === 'jadwal' && (
                                    <div className="space-y-6">
                                        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                                            ...
                                            <p className="text-gray-500 text-sm">Berikan tantangan kepada siswa untuk dikerjakan. Misi akan muncul di dashboard mereka.</p>
                                        </div>
                                                ...
                                                <div key={idx} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all flex justify-between items-center group">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-[10px] text-gray-400">Jadwalkan misi</p>
                                                        </div>
                                                    </div>
                                                    <button 
                                                        onClick={() => openMissionModal(habit.title)}
                                                        className="px-4 py-2 bg-gray-100 text-gray-600 font-bold text-xs rounded-lg hover:bg-emerald-100 hover:text-emerald-700 transition-colors uppercase tracking-wide"
                                                    >
                                                        Buat Misi
                                                    </button>
                                                </div>
                            ...
                            <div className="space-y-6 animate-fade-in">
                                <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-end">
                                    <div>
                                        ...
                                        <p className="text-gray-500 text-sm">Daftar aktivitas penilaian yang telah Anda lakukan.</p>
                                    </div>
                                </div>

                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    {/* Kalender (Visual Only for now) */}
                                    ...
                                    <div className="bg-white p-6 rounded-3xl shadow-lg border border-gray-100 lg:col-span-2">
                                        ...
                                        <div className="overflow-y-auto max-h-[400px] space-y-3 pr-2 custom-scrollbar">
                                            {historyData.length === 0 ? (
                                                <p className="text-center text-gray-400 text-sm py-10">Belum ada riwayat aktivitas.</p>
                                            ) : (
                                                    ...
                                                    <div key={h.id} className="p-4 bg-gray-50 rounded-xl flex justify-between items-center group hover:bg-emerald-50 transition-colors">
                                                        <div>
                                                            <div className="flex gap-2 mb-1">
                                                                <span className="text-[10px] font-bold bg-white border border-gray-200 px-2 py-0.5 rounded text-gray-500">
                                                                    {new Date(h.record_date).toLocaleDateString('id-ID')}
                                                                </span>
                                                                <span className="text-[10px] font-bold px-2 py-0.5 rounded uppercase bg-blue-100 text-blue-700">
                                                                    {h.type}
                                                                </span>
                                                            </div>
                                                            <p className="font-bold text-gray-800 text-sm">{h.student_name} <span className="text-gray-400 text-xs">({h.class})</span></p>
                                                        ...
                                                        <div className="text-right">
                                                            ...
                                                            <span className="text-[10px] font-bold text-gray-400">Poin</span>
                                                        </div>
                                                    </div>
                                                ))
                                            )}
                    ...
                    <div className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl scale-100 transform transition-all">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input 
                                    type="text" 
                                    placeholder="Contoh: Lari keliling lapangan 3x" 
                            ...
                            <div>
                                ...
                                <div className="flex gap-2 mb-2">
                                    <button 
                                        onClick={() => setMisiForm({...misiForm, targetType: 'siswa', targetId: ''})}
                                        className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${misiForm.targetType === 'siswa' ? 'bg-emerald-600 text-white shadow-md' : 'bg-gray-100 text-gray-500'}`}
                                    >
                                        <UserCircle size={14} className="inline mr-1"/> Perorangan
                                    </button>
                                    <button 
                                        onClick={() => setMisiForm({...misiForm, targetType: 'kelas', targetId: ''})}
                                        className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${misiForm.targetType === 'kelas' ? 'bg-emerald-600 text-white shadow-md' : 'bg-gray-100 text-gray-500'}`}
                                    >
                                ...
                                {misiForm.targetType === 'siswa' ? (
                                    <select 
                                        className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl font-bold text-gray-700 outline-none"
                            ...
                            <div>
                                <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">Tenggat Waktu (Due Date)</label>
                                <input 
                                    type="date" 
                                    className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl font-bold text-gray-700 outline-none"
                            ...
                            <button 
                                onClick={handleJadwalkanMisi}
                                className="w-full bg-emerald-600 text-white py-3.5 rounded-xl font-bold text-lg hover:bg-emerald-700 transition-all shadow-lg hover:shadow-emerald-200 mt-4 flex justify-center items-center gap-2"
                            >

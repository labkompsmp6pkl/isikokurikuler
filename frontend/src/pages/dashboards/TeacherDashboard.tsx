...
const TeacherDashboard: React.FC = () => {
    ...
    // Ambil User & NIP dari LocalStorage
    ...
    // Logout dengan Hard Refresh untuk membersihkan state
    const handleLogout = () => {
    ...
    // Ambil data siswa yang sedang dipilih (termasuk parent_name dari backend)
    ...
    if (errorMsg) return (
        <div className="h-screen flex flex-col justify-center items-center bg-gray-50 p-6 text-center">
            <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-gray-200">
                ...
                <button onClick={handleLogout} className="w-full flex justify-center items-center gap-2 bg-red-600 text-white py-3 rounded-xl font-bold hover:bg-red-700 transition-colors shadow-lg">
        ...
        <div className="min-h-screen bg-[#F3F4F6] font-sans flex text-gray-800">
            ...
            <style>{`
                @media print {
                    ...
                    /* Reset warna background saat print */
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                <header className="bg-white border-b border-gray-200 p-4 flex justify-between items-center md:hidden sticky top-0 z-10 no-print">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-gray-800">ISIKOKURIKULER</span>
                    </div>
                ...
                <main className="flex-1 overflow-auto p-4 md:p-8">
                    <div className="max-w-6xl mx-auto pb-20">
                        ...
                        {activeTab === 'beranda' && (
                            <div className="space-y-8 animate-fade-in">
                                <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
                                    ...
                                    <p className="text-indigo-100 max-w-2xl text-lg font-medium relative z-10">Panel kontrol ISIKOKURIKULER untuk kelas {data?.teacherClass}.</p>
                                </div>
                                <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2"><span className="w-1 h-6 bg-indigo-600 rounded-full"></span> Indikator Penilaian Karakter</h3>
                        ...
                        {activeTab === 'validasi' && (
                            <div className="space-y-6 animate-fade-in">
                                <div className="flex flex-col md:flex-row justify-between items-end gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                                    <div><h2 className="text-2xl font-black text-gray-800">Validasi Jurnal</h2><p className="text-gray-500 font-medium mt-1">Sahkan kegiatan siswa.</p></div>
                                    <div className="text-right"><span className="block text-3xl font-black text-indigo-600">{pendingCount}</span><span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Menunggu Validasi</span></div>
                                </div>
                                    ...
                                    {logs.map((log: any) => {
                                        ...
                                        return (
                                            <div key={log.id} className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row gap-5 items-start md:items-center">
                                                <div className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 ${isReady ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600'}`}>{isReady ? <CheckSquare size={24} /> : <Clock size={24} />}</div>
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex flex-wrap gap-2 mb-1"><span className="bg-gray-100 text-gray-600 text-[10px] font-bold px-2 py-0.5 rounded uppercase">{formatDateIndo(log.log_date)}</span><span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase ${isReady ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{log.status}</span></div>
                                                ...
                                                <div className="flex gap-2 w-full md:w-auto">
                                                    <button onClick={() => setSelectedLog(log)} className="flex-1 md:flex-none px-4 py-2.5 text-sm font-bold text-indigo-600 bg-indigo-50 rounded-xl hover:bg-indigo-100">Lihat</button>
                                                    <button onClick={() => handleValidate(log.id)} disabled={!isReady} className={`flex-1 md:flex-none px-4 py-2.5 text-sm font-bold text-white rounded-xl shadow-md transition-all ${isReady ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-gray-300 cursor-not-allowed grayscale opacity-70'}`}>{isReady ? '✅ Sahkan' : '⏳ Menunggu'}</button>
                                                </div>
                                            </div>
                                        );
                                    })}
                                    {logs.length === 0 && <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-300"><div className="bg-gray-50 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 text-gray-400"><CheckSquare size={32} /></div><p className="font-bold text-gray-500">Semua tugas validasi sudah selesai!</p></div>}
                                </div>
                            </div>
                        )}
                        ...
                        {activeTab === 'riwayat' && (
                            <div className="space-y-6 animate-fade-in">
                                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col lg:flex-row justify-between items-center gap-4">
                                    <div className="w-full"><h2 className="text-2xl font-black text-gray-800">Kalender Kelas</h2><p className="text-gray-500 text-sm">Filter aktivitas siswa.</p></div>
                                    <select className="w-full lg:w-64 border border-gray-300 p-2.5 rounded-xl font-semibold text-gray-700 outline-none" value={filterStudentId} onChange={(e) => setFilterStudentId(e.target.value)}><option value="">-- Semua Siswa --</option>{data?.students.map((s:any) => (<option key={s.id} value={s.id}>{s.full_name}</option>))}</select>
                                </div>
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                    <div className="bg-white p-6 rounded-3xl shadow-lg border border-gray-100">
                                        <Calendar locale="id-ID" className="w-full border-none font-sans" tileClassName={({ date }) => historyLogs.some((l:any) => l.log_date.startsWith(formatISODate(date))) ? 'bg-indigo-50 text-indigo-700 font-bold rounded-lg' : null} onClickDay={(date) => { const log = historyLogs.find((l:any) => l.log_date.startsWith(formatISODate(date))); if(log) setSelectedLog(log); else toast('Tidak ada data', { icon: '📅' }); }} />
                                    </div>
                                    <div className="bg-white p-6 rounded-3xl shadow-lg border border-gray-100 flex flex-col h-[500px]">
                                        <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Clock size={18} className="text-indigo-600"/> Riwayat Terkini</h3>
                                        <div className="overflow-y-auto flex-1 pr-2 space-y-3 custom-scrollbar">
                                            {historyLogs.slice(0, 20).map((h: any) => (
                                                <div key={h.id} onClick={() => setSelectedLog(h)} className="group p-4 rounded-2xl bg-gray-50 hover:bg-indigo-50 border border-transparent hover:border-indigo-100 cursor-pointer transition-all">
                                                    <div className="flex justify-between items-start mb-1"><span className="font-bold text-gray-800 text-sm">{h.student_name}</span><span className="text-[10px] text-gray-400 font-medium">{formatDateIndo(h.log_date)}</span></div>
                                                    <div className="flex items-center justify-between"><span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${h.status === 'Disahkan' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'}`}>{h.status}</span><ChevronRight size={16} className="text-gray-300 group-hover:text-indigo-400"/></div>
                                                </div>
                                            ))}
                        ...
                        {/* TAB ANALISIS (AREA YANG DIPERBAIKI) */}
                        {activeTab === 'analisis' && (
                            <div className="space-y-8 animate-fade-in">
                                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-200 no-print">
                                    <div className="flex items-center gap-3 mb-6"><div className="p-3 bg-indigo-100 text-indigo-600 rounded-xl"><Sparkles size={24}/></div><div><h2 className="text-2xl font-black text-gray-800">Generator Rapor AI</h2><p className="text-gray-500 text-sm">Analisis otomatis perkembangan karakter siswa.</p></div></div>
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-5 items-end">
                                        <div className="space-y-2"><label className="text-xs font-bold text-gray-500 uppercase">Siswa</label><select className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl font-bold text-gray-700 outline-none" value={reportConfig.studentId} onChange={(e) => setReportConfig({ ...reportConfig, studentId: e.target.value })}><option value="">-- Pilih Siswa --</option>{data?.students.map((s:any) => (<option key={s.id} value={s.id}>{s.full_name}</option>))}</select></div>
                                        <div className="space-y-2"><label className="text-xs font-bold text-gray-500 uppercase">Dari Tanggal</label><input type="date" className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl font-bold text-gray-700 outline-none" value={reportConfig.startDate} onChange={(e) => setReportConfig({ ...reportConfig, startDate: e.target.value })}/></div>
                                        <div className="space-y-2"><label className="text-xs font-bold text-gray-500 uppercase">Sampai Tanggal</label><input type="date" className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl font-bold text-gray-700 outline-none" value={reportConfig.endDate} onChange={(e) => setReportConfig({ ...reportConfig, endDate: e.target.value })}/></div>
                                        <button onClick={handleGenerateReport} disabled={isGenerating} className="h-[50px] bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-all flex justify-center items-center gap-2 shadow-lg shadow-indigo-200 disabled:opacity-70">{isGenerating ? <Spinner /> : <><Sparkles size={18}/> Analisis Sekarang</>}</button>
                                    </div>
                                </div>
                                ...
                                {analysisResult && (
                                    <div ref={printRef} className="print-area animate-slide-up">
                                        <div className="bg-white p-6 rounded-t-3xl border-b border-gray-100 flex justify-between items-center no-print shadow-sm">
                                            ...
                                            <button onClick={handlePrint} className="bg-gray-900 text-white px-5 py-2 rounded-lg font-bold text-sm hover:bg-black transition-colors flex items-center gap-2"><Printer size={16} /> Cetak PDF Resmi</button>
                                        </div>
                                        ...
                                        <div className="bg-white p-8 md:p-12 rounded-b-3xl shadow-xl border border-gray-100 print:shadow-none print:border-none print:p-0">
                                            {/* KOP SURAT TERBARU DENGAN LOGO */}
                                            <div className="flex items-center justify-center border-b-4 border-double border-black pb-4 mb-8 gap-4">
                                                <img src="/logo-smpn6.png" alt="Logo SMPN 6" className="w-24 h-auto object-contain" />
                                                <div className="text-center">
                                                    <h1 className="text-3xl font-black text-gray-900 uppercase tracking-widest mb-1">SMP NEGERI 6 PEKALONGAN</h1>
                                                    <p className="text-sm font-bold text-gray-600 uppercase tracking-widest">Laporan Perkembangan Karakter Siswa (ISIKOKURIKULER)</p>
                                                </div>
                                            </div>
                                            
                                            <table className="w-full text-left text-sm font-medium mt-6 mb-8">
                                                <tbody>
                                                    <tr>
                                                        <td className="w-32 py-1 font-bold">Nama Siswa</td><td className="w-4">:</td><td className="font-bold">{selectedStudent?.full_name}</td>
                                                        <td className="w-32 text-right font-bold">Kelas</td><td className="w-4 text-center">:</td><td className="w-24 font-bold">{data.teacherClass}</td>
                                                    </tr>
                                                    <tr>
                                                        <td className="py-1 font-bold">Periode</td><td>:</td><td colSpan={4}>{formatDateIndo(reportConfig.startDate)} s/d {formatDateIndo(reportConfig.endDate)}</td>
                                                    </tr>
                                                </tbody>
                                            </table>

                                            <div className="space-y-8 font-serif text-gray-900 leading-relaxed text-justify">
                                                <section><h4 className="font-bold text-lg mb-2 underline decoration-2 decoration-orange-300 underline-offset-4">1. Ringkasan Eksekutif</h4><p>{analysisResult.executive_summary}</p></section>
                                                <section><h4 className="font-bold text-lg mb-2 underline decoration-2 decoration-blue-300 underline-offset-4">2. Progress Karakter</h4><p>{analysisResult.character_progress}</p></section>
                                                <section><h4 className="font-bold text-lg mb-2 underline decoration-2 decoration-purple-300 underline-offset-4">3. Catatan Wali Kelas (Narasi Rapor)</h4><div className="bg-gray-50 p-6 rounded-xl border-l-4 border-indigo-500 italic print:bg-transparent print:border-l-2 print:border-black print:p-2">"{analysisResult.report_narrative}"</div></section>
                                                
                                                {/* TANDA TANGAN */}
                                                <div className="flex justify-between mt-16 pt-8 break-inside-avoid">
                                                    <div className="text-center w-64">
                                                        <p className="mb-20">Mengetahui,<br/>Orang Tua/Wali</p>
                                                        {/* NAMA ORANG TUA OTOMATIS */}
                                                        <p className="border-t border-black pt-1 font-bold">{selectedStudent?.parent_name ? selectedStudent.parent_name : "( ........................................ )"}</p>
                                                    </div>
                                                    <div className="text-center w-64">
                                                        <p className="mb-20">Pekalongan, {new Date().toLocaleDateString('id-ID', {day: 'numeric', month: 'long', year: 'numeric'})}<br/>Wali Kelas</p>

...
const StudentLayout: React.FC = () => {
  ...
  const navItems = [
    { path: '/student/beranda', label: 'Beranda', icon: <LayoutDashboard size={20} /> },
    { path: '/student/journal', label: 'Isi Jurnal', icon: <PenTool size={20} /> },
    { path: '/student/misi', label: 'Misi', icon: <Target size={20} /> }, 
    ...
    <div className="min-h-screen bg-gray-50 flex font-sans text-slate-800">
      ...
      <aside className={`fixed md:sticky top-0 h-screen w-72 bg-white border-r border-gray-200 z-50 transition-transform duration-300 ease-in-out flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50/50">
          <div className="bg-white p-4 rounded-[1.5rem] border border-gray-200 shadow-sm mb-4 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="md:hidden sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-gray-200 p-4 shadow-sm transition-all">
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-gray-50 mt-2">
            <div className="w-9 h-9 rounded-xl bg-blue-600 text-white flex items-center justify-center font-black text-xs shadow-md uppercase">
              {userData.initial}
            </div>
